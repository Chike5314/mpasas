...
def create_app():
    ...
    def scan_answer_key(tid):
        ...
        if request.method == 'POST':
            ...
            def sort_key(item):
                key = item[0]
                # Extract numeric part from key (e.g., "Q1" -> 1)
                num_str = ''.join(c for c in key if c.isdigit())
                return int(num_str) if num_str else 0
            
            scanned_key_sorted = dict(sorted(scanned_answers.items(), key=sort_key))
            scanned_key_json_sorted = json.dumps(scanned_key_sorted)

            return render_template('scan_key.html',
                                   template=tmpl,
                                   scanned_key=scanned_key_sorted,
                                   scanned_key_json=scanned_key_json_sorted,
                                   warnings=warnings,
                                   align_note=note,
