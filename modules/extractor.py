...
def analyse_grid(
    ...
    Adaptive threshold: automatically adjusts based on cell size for robust detection
    across small and large bubble grids.
    ...
    fill_threshold  : minimum dark-pixel ratio to count as 'marked' (0-1 scale)
    ...
    adaptive_threshold = fill_threshold
    if cell_h * cell_w > 0:
        cell_pixels = cell_h * cell_w
        if cell_pixels < 500:
            adaptive_threshold = max(0.08, fill_threshold - 0.02)
        elif cell_pixels > 2000:
            adaptive_threshold = min(0.15, fill_threshold + 0.02)
    ...
    for r in range(rows):
        ...
        marked_cols = [i for i, rat in enumerate(fill_ratios) if rat >= adaptive_threshold]

        if max_r < adaptive_threshold:
            results[q_num] = {
                'answer': None,
                'confidence': 0.0,
        ...
        confidence_gap = max_r - second_r
        if len(marked_cols) > 1 and confidence_gap < 0.08:
            status = 'ambiguous'
...
def extract_all_answers(
    ...
    Supports multi-zone layouts (e.g., 25+25 questions in left/right columns).
    ...
    for zone in zones:
        ...
        if rows > 100:
            warnings.append(f"Zone '{zone.get('name', '?')}': {rows} rows detected (large sheet)")
        ...
        for local_q, result in grid.items():
            ...
            if result['status'] == 'ambiguous':
                warnings.append(f"{global_q}: multiple marks detected – left ungraded")
